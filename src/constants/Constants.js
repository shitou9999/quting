/**
 * Created by cyh on 2018/8/7.
 */
// import * as Constants from '../constants/Constants'
export const baseUrl = 'http://192.168.200.153:2080/uc_app-inf'
// const baseUrl = 'http://beta..cc:32080/_app-inf'
export const upUrl = 'http://192.168.200.153:2080/uc_file-srv/upload.do?file='
export const loadUrl = 'http://192.168.200.153:2080/uc_file-srv/download.do?file='

//推送
export const PUSH_ALIAS_TYPE = "UCHOICE"

//home
export const Emitter_SELECT_REFRESH = 'refresh'
export const Emitter_WALLET_REFRESH ='WalletChange'
//欠费补缴支付成功刷新msg
export const Emitter_ARREARS_REFRESH = 'OverduePayPage'
export const Emitter_BIND_REFRESH = 'bind'

// public static final String PREF_ID = "id";//用户id
// public static final String PREF_USERCODE = "userCode";
// public static final String PREF_USERPWD = "userPwd";
// public static final String PREF_USERNAME = "userName";
// public static final String PREF_NICKNAME = "nickName";
// public static final String PREF_SEX = "sex";
// public static final String PREF_USERPIC = "userPic";
// public static final String PREF_CLIENTID = "clientId";
// public static final String PREF_PAYPWD = "payPwd";
// public static final String PREF_ISAUTO = "isAuto";
// public static final String PREF_TOKEN = "token";
// public static final String SHOW_GUIDE = "guide";
// public static final String LOGIN_FIRST = "login_first";
// public static final String DEVICETOKEN = "deviceToken";
// public static final String BIND_ERROR = "100206";

